...
def get_activity_by_action(cur_time_str, action, pre_act = '', pre_act_symbol_list = [], current_activity_duration = None):
    ...
    # cur_time_str = cur_time.strftime(rl_env_ascc_privacy_test.DATE_HOUR_TIME_FORMAT)
    ...
    try:
        ...
        cur_g_image_recognition_time_str = cur_g_image_recognition_time.strftime(rl_env_ascc_privacy.DATE_HOUR_TIME_FORMAT)
        ...
        tools_sql.insert_adl_activity_data(activity, time_act, image_source, sound_source, motion_source)
        print('insert int to db: activity get activity:', activity, ' cur_time:', cur_time_str, ' real time:', time_act)
    except Exception as e:
        print('insert int to db: activity get activity fail', e)
        pass
    ...
    #     audio_data, vision_data, motion_data, transition_motion = env.step(rl_env_ascc_privacy_test.WMU_FUSION_ACTION)
    ...
    #     detected_activity, _ = get_activity_by_action(cur_time_str, rl_env_ascc_privacy_test.WMU_FUSION_ACTION)
...
def real_time_test_run():
    ...
    state_size = 29
    agent = rl_ascc_dqn.DQNAgent(state_size, action_space, episodes=500*10, epsilon = 0.00001, memory_size = 1280)
    ...
    for episode in range(episode_count):
        ...
        while(pre_activity == ''):
            ...
            # audio_data, vision_data, motion_data, transition_motion = env.step(rl_env_ascc_privacy.ROBOT_FUSION_ACTION)
            ...
            #     env.step(rl_env_ascc_privacy.WMU_FUSION_ACTION)
            #     time2 = timer()
            #     duration = time2 - time1
            ...
            detected_activity, location = get_activity_by_action(cur_time_str, rl_env_ascc_privacy.WMU_FUSION_ACTION, pre_act = '', pre_act_symbol_list=[])
            if detected_activity == '':
                continue
        ...
        while(not env.done):
            ...
            if rl_env_ascc_privacy.RL_ACTION_DICT[action] == rl_env_ascc_privacy.WMU_fusion  \
                or rl_env_ascc_privacy.RL_ACTION_DICT[action] == rl_env_ascc_privacy.Robot_audio_vision \
                    or rl_env_ascc_privacy.RL_ACTION_DICT[action] == rl_env_ascc_privacy.Robot_audio_WMU_vision:
                if check_and_wait_l_o_s_m_result() == False:
                    # pass
                    continue

            if rl_env_ascc_privacy.RL_ACTION_DICT[action] == rl_env_ascc_privacy.WMU_vision  \
                or rl_env_ascc_privacy.RL_ACTION_DICT[action] == rl_env_ascc_privacy.Robot_vision:
            ...
            # print("Env action: ", action, " ", rl_env_ascc_privacy_test.RL_ACTION_DICT[action])

            # env check the action and the cost time
            # action = 6 # rl_env_ascc_privacy_test.Robot_WMU_fusion # to get the time recognition dict
            ...
            # if rl_env_ascc_privacy_test.RL_ACTION_DICT[action] == rl_env_ascc_privacy_test.Nothing:
            #     detected_activity = pre_activity 
            # elif rl_env_ascc_privacy_test.RL_ACTION_DICT[action] == rl_env_ascc_privacy_test.Robot_audio_vision:
            #     detected_activity = ground_truth_activity
            # else:
            #     # Todo 
